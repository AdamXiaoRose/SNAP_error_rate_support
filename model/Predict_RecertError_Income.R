...
ols_data <- df_recert_income %>% select("counted_income_error_i","element",all_of(features))
...
set.seed(111)
cv_set <- vfold_cv(model_data, v = 3)
...
rf_recipe <- recipe(income_error_c ~ ., data = model_data)
names(model_data)
...
#  use_strata <- F
  use_strata <- n_distinct(data$income_error_c) > 1

  split <- if (use_strata) {
    initial_split(data, prop=3/4, strata=income_error_c)
  } else {
    initial_split(data)
  }
  ...
  folds <- if (use_strata) {
    vfold_cv(train, v = 5, strata = NULL)
  } else {
    vfold_cv(train, v = 5)
  }
  ...
  preds <- predict(fit, test, type = "prob") %>%
    bind_cols(test %>% select(income_error_c))
  
  auc <- roc_auc(
    data = preds,
    truth = income_error_c,
    c(.pred_no_error, .pred_non_income_error, .pred_over_threshold, .pred_under_threshold)
  )
...
levels(model_data$income_error_c)
#############################################
### 7. Run models (Certification errors)
#############################################

multinom_recert_income <- run_ml(model_data_SO, multinom_spec, "multinom_recert")
multinom_recert_income$auc

table(model_data$income_error_c)
rf_recert <- run_ml(model_data, rf_spec, "RF_recert")
rf_recert$auc

rf_recert_SO <- run_ml(model_data_SO, rf_spec, "RF_recert")
rf_recert_SO$auc

rf_recert_SandC <- run_ml(model_data_SandC, rf_spec, "RF_recert")
rf_recert_SandC$auc

rf_fit_parsnip <- extract_fit_parsnip(rf_recert_SO$fit)
vip(rf_fit_parsnip, num_features = 100) 

rf_recert$fit$fit$fit$fit$forest$independent.variable.names

model_data$expedited_i <- as.numeric(model_data$expedited_i=="TRUE")

#optimized at 
boost_recert <- run_ml(model_data, boost_spec, "Boost_recert")  
boost_recert$auc
...
quantile(model_data$rawben_rel_max, p=c(.29,.59))
#income ratio:
...
summary(model_data[model_data$state=="Washington",])
